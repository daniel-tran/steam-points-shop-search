 ...
 */
export function getConfigData(responseData: string, urlPrefix: string, urlLimit: number, idExtractionRegex: RegExp = /\d+/) {
    const parsedData = cheerio.load(responseData);
    parsedData.html();
    const options = parsedData('.btn-primary.p-2');
    const appNames = parsedData('.mr-2.tracking-wider.truncate.font-league-gothic');
